...
jQuery(function() {
  var clear_repin_form_notifications, disableMiddleMouseButtonScrolling, disableNormalScroll, disable_scroll, enable_scroll, get_more_items, get_next_column, get_next_column_2, open_pin_detail, show_error, simplify_url, update_next_column;
  $.ajaxSetup({
    cache: false
  });
  get_more_items = function(start) {
    ...
    $.getJSON('', {
      ...
      var column_control, html_text, pin, _i, _len;
  ...
  get_next_column = function() {
    var column1, column4, column5, columns, i;
    columns = [0, 0, 0, 0, 0];
    $('#category_column_1 div').each(function() {
      return columns[0] += $(this).height();
    });
    column1 = 2;
    $('#category_column_2 div').each(function() {
      return columns[1] += $(this).height();
    });
    column1 = 3;
    $('#category_column_3 div').each(function() {
      return columns[2] += $(this).height();
    });
    column4 = 0;
    $('#category_column_4 div').each(function() {
      return columns[3] += $(this).height();
    });
    column5 = 0;
    $('#category_column_5 div').each(function() {
      return columns[4] += $(this).height();
    });
    i = columns.indexOf(Math.min.apply(Math, columns));
    return i + 1;
  };
  get_next_column_2 = function() {
    var i, min_column, min_value, value, _i;
    min_column = 1;
  ...
  update_next_column = function(column, value) {
